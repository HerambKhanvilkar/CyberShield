...
const VerticalsDropDown = ({ verticals, formData, handleChange }) => {
  ...
  const filteredSkills = Array.isArray(verticals)
    ? verticals.filter((vertical) =>
        vertical.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : [];
  ...
  <input type="text" value={searchTerm} onFocus={handleFocus} onChange={handleSearchChange} className="flex-1 bg-transparent outline-none text-white placeholder:text-gray-400 px-3 py-2" placeholder="Search vertical" />
      }
